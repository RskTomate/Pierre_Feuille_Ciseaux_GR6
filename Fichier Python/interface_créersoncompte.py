"""
fenetre_inscription.py — Création de compte connectée au serveur.
"""
...
def shadow(draw, pos, text, font, color="#FFFCF6", sh="#2C1A0E"):
    draw.text((pos[0]+2, pos[1]+2), text, font=font, fill=sh)
    draw.text(pos, text, font=font, fill=color)

shadow(draw, (60, 40),  "Créer un compte",     font_titre)
shadow(draw, (40, 120), "Pseudo",               font_label)
shadow(draw, (40, 210), "Mot de passe",         font_label)
shadow(draw, (40, 300), "Valider mot de passe", font_label)
...
entry_pseudo = ctk.CTkEntry(app, width=200, height=35, **entry_style)
entry_pseudo.place(x=40, y=155)

entry_mdp  = ctk.CTkEntry(app, width=300, height=35, show="*", **entry_style)
entry_mdp.place(x=40, y=245)

entry_mdp2 = ctk.CTkEntry(app, width=300, height=35, show="*", **entry_style)
entry_mdp2.place(x=40, y=335)

label_statut = ctk.CTkLabel(app, text="", font=("Arial", 12, "bold"),
                             fg_color="transparent", text_color=ROUGE)
label_statut.place(x=40, y=385)
...
def valider():
    ...
    mdp2   = entry_mdp2.get()
    if not pseudo:
        label_statut.configure(text="✗ Pseudo vide.", text_color=ROUGE); return
    if len(pseudo) < 3:
        label_statut.configure(text="✗ Pseudo trop court (min. 3).", text_color=ROUGE); return
    if len(mdp) < 4:
        label_statut.configure(text="✗ Mot de passe trop court (min. 4).", text_color=ROUGE); return
    if mdp != mdp2:
        label_statut.configure(text="✗ Mots de passe différents.", text_color=ROUGE); return
    if not client.connected:
        label_statut.configure(text="✗ Serveur inaccessible.", text_color=ROUGE); return
    label_statut.configure(text="Envoi…", text_color=TEXTE_FONCE)
...
ctk.CTkButton(app, text="Valider", font=("Arial", 22, "bold"),
              ...
              command=valider).place(x=40, y=420)

ctk.CTkButton(app, text="Se connecter", font=("Arial", 13),
              text_color=BRUN_BOIS, width=50, height=10,
              fg_color=PARCHEMIN, hover_color=OR, corner_radius=6,
              command=ouvrir_connexion).place(x=250, y=455)
